...
import { getInitials, toTitleCase } from '../../utils/helpers'
    ...
    <div className="max-w-[600px]">
      ...
      <div className="flex items-center gap-4 mb-8
        ...
        <div className="w-16 h-16 rounded-full bg-accent/20
                        ...
                        text-accent-lt text-[22px] font-bold flex-shrink-0">
          {getInitials(user?.name)}
        </div>
        <div>
          <p className="text-text font-bold text-lg m-0">{user?.name}</p>
          <p className="text-muted text-[13px] mt-1 mb-0">{user?.email}</p>
          <span className="bg-accent/10 text-accent-lt text-[11px] px-2.5 py-0.5
                           rounded-full font-semibold inline-block mt-1.5">
